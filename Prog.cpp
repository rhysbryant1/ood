...
istream& operator>> (istream& in, Prog& prg)
{
	...
	for (int a(0); a < num_instructions; a++) {
		
		in >> instruction_type;		// Store instruction type
		...
		if (instruction_type == "FORWARD") {
			...
			list[i] = (Instruction*)fwd;		// Store Instruction into vector (of type Instruction not Forward)
		}
			...
			list[i] = (Instruction*)jmp;		// Cast Instruction into vector (type is Instruction not Jump)
		}
			...
			list[i] = (Instruction*)lft;		// Store Instruction into vector (of type Instruction not Left)
		}
			...
			list[i] = (Instruction*)rgt;		// Store Instruction into vector (of type Instruction not Right)
		}
			...
			list[i] = (Instruction*)rpt;		// Store Instruction into vector (of type Instruction not Repeat)
		}
